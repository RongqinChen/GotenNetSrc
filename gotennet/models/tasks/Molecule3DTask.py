"""Molecule3D task implementation for quantum chemistry property prediction."""
...
from gotennet.datamodules.components.molecule3d import Molecule3D
...
class Molecule3DTask(Task):
    """
    Task for the Molecule3D quantum chemistry dataset.

    This task predicts various quantum chemistry properties for molecules
    in the Molecule3D dataset, including HOMO, LUMO, HOMO-LUMO gap,
    SCF energy, and dipole moment components.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the Molecule3D task.

        Args:
            representation: The representation model to use.
            label_key: The key or index for the label in the dataset.
            dataset_meta: Metadata about the dataset (e.g., mean, std, atomref).
            task_config: Configuration for the task. Defaults to None.
    ...
    def process_outputs(
        ...
        Args:
            ...
            result: The dictionary containing model outputs (predictions).
            metric_meta: Metadata about the metric, including 'prediction'
                and 'target' keys.
            metric_idx: Index of the metric.

        Returns:
            A tuple containing the processed predictions and targets.
        """
    ...
    def get_metric_names(
        ...
        Args:
            ...
            metric_idx: Index of the metric. Defaults to 0.

        Returns:
            The name of the metric, potentially including the property name.
        """
    ...
    def get_losses(self) -> list[dict]:
        """
        Get the loss functions for the Molecule3D task.

        Returns:
            A list of dictionaries, each containing loss function configuration.
        """
    ...
    def get_metrics(self) -> list[dict]:
        """
        Get the metrics for the Molecule3D task.

        Returns:
            A list of dictionaries, each containing metric configuration.
        """
        return [
            {
    ...
    def get_output(self, output_config: dict | None = None) -> torch.nn.ModuleList:
        """
        Get the output module for the Molecule3D task.

        Uses the Atomwise output module for all properties, similar to
        the default QM9 properties.

        Args:
            output_config: Configuration for the output module.

        Returns:
            A ModuleList containing the appropriate output module.
        """
    ...
    def get_evaluator(self) -> None:
        """
        Get the evaluator for the Molecule3D task.
    ...
    def get_dataloader_map(self) -> list[str]:
        """
        Get the dataloader map for the Molecule3D task.

        Returns:
            A list containing 'test' as the only dataloader phase to use.
        """
        return ["test"]
